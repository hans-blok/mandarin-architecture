...
DEBUG = False  # Enable debug output to see what's happening
...
def load_manifest(repo_path: Path, manifest_name: str) -> Tuple[List[AgentSpec], Dict[str, str], Dict[str, str]]:
    ...
    if DEBUG:
        print(f"\n[DEBUG] Locaties in manifest:")
        for key, value in locaties.items():
            print(f"  {key}: {value}")
    ...
    for idx, entry in enumerate(agents_raw):
        ...
        if charter_tpl:
            charter_path = Path(charter_tpl.replace("<value-stream>", value_stream).replace("<agent-naam>", naam))
            files.append(charter_path)
            if DEBUG:
                print(f"[DEBUG] Agent {naam}: charter path = {charter_path}")
        # prompts (wildcard)
        ...
        for rel in spec.files:
            ...
            if "*" in rel.name:
                ...
                if not abs_path.exists():
                    ...
                    if DEBUG:
                        print(f"[DEBUG] NOT FOUND: {abs_path}")
                    continue
                if DEBUG:
                    print(f"[DEBUG] FOUND: {abs_path}")
                ...
                if abs_path.is_dir() and "runners" in abs_path.parts:
                    ...
                    # Non-runner directory, copy all relevant files inside
                    if "charters" in abs_path.parts or "charters-agents" in abs_path.parts:
                        found_charters = list(abs_path.rglob("*.md"))
                        if DEBUG:
                            print(f"[DEBUG] Directory {abs_path} contains {len(found_charters)} .md files")
                        bucket.extend(found_charters)
                    else:
...
def organize(vs_files: List[Path], util_files: List[Path], runner_modules: List[Path], workspace: Path, repo_path: Path) -> Dict[str, int]:
    ...
    for src in all_files:
        dest = None
        if src.suffix == ".md":
            if "charters" in src.parts or "charters-agents" in src.parts or "charter" in src.name.lower():
...
def main() -> int:
    ...
    value_stream = args.value_stream.strip("'\"") if isinstance(args.value_stream, str) else args.value_stream
    ...
    try:
        ...
        # Sync this script from source of truth (mandarin-agents)
        ...
        # Note: The mandarin-agents folder is deliberately left in place.
        # It will be reused with 'git pull' on the next run, which is more efficient
        # than cloning from scratch every time.
